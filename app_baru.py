...
with st.sidebar:
    ...
    DATABASE_ID = {
        ...
        "AUGUST": "1Rfjz1JeZG47OtcDr-lQYZqGuzvn2l4OgwqgmS4TG0kA",
        "SEPTEMBER": "17_pezGi3WJQwTSj_vmzs09RzZZR8oF8O1djGLdFx5DQ",
...
with col_teks:
    ...
    st.markdown(f'<div class="sub-header"> <b>{sheet_name}</b> | Production, Quality & Efficiency</div>', unsafe_allow_html=True)
    st.markdown('<div class="dev-credit">✨ Dev : Mahesya</div>', unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)
st.divider()

# ==========================================
# 📘 KAMUS SPEK (RENTANG pH DIPERBARUI SESUAI PERMINTAAN)
# ==========================================
KAMUS_SPEK_PH = { 
    "Z 125": [3.0, 5.0], 
    "Z 211": [3.5, 6.0], 
...
# ==========================================
# 🧾 FUNGSI PDF GENERATOR (VERSI 1 HALAMAN PRESISI)
# ==========================================
def create_pdf(sheet_name, df_pdf, total_prod, achievement, forecast_text):
    ...
    class PDF(FPDF):
        def header(self):
            ...
            self.cell(26) # Jarak dari Logo agar pas
            ...
            # --- FIX GARIS BERTUMPUK LOGO ---
            # Posisi Y diturunkan ke 31 agar aman persis di bawah logo
            self.set_draw_color(0, 91, 234) 
            self.set_line_width(0.8)
            self.line(10, 31, 200, 31)
            
            self.set_draw_color(0, 0, 0) 
            self.set_line_width(0.2)
            self.line(10, 32.5, 200, 32.5)
            self.ln(11) # Jarak agar Judul tidak nempel garis
        ...
        # Footer Dihapus Pagenumbernya agar tidak memakan limit bawah (Muat 1 halaman)
        def footer(self):
            self.set_y(-8)
            self.set_font('Arial', 'I', 6)
            self.set_text_color(150, 150, 150)
            self.cell(0, 10, safe_text('Generated by Mahesya System Dashboard'), 0, 0, 'R')

    pdf = PDF()
    
    # --- PENGATURAN MARGIN AGAR PAS 1 HALAMAN ---
    pdf.set_margins(10, 10, 10)
    pdf.set_auto_page_break(auto=True, margin=10) # Set margin bawah jadi minimal
    pdf.add_page()
    
    # KOTAK RINGKASAN TOTAL PRODUKSI
    pdf.set_font('Arial', 'B', 8)
    pdf.set_text_color(0, 0, 0)
    pdf.set_fill_color(240, 248, 255) 
    pdf.set_draw_color(200, 200, 200)
    pdf.cell(0, 5, safe_text(f'   Total Production: {total_prod:,.1f} Ton   |   Target Achievement: {achievement:.1f}%'), 1, 1, 'L', fill=True)
    pdf.ln(2)
    
    # TABEL HEADER (KOLOM BIRU)
    pdf.set_font('Arial', 'B', 7)
    pdf.set_fill_color(0, 91, 234) 
    pdf.set_text_color(255, 255, 255) 
    pdf.set_draw_color(180, 180, 180) 
    
    # Pengaturan Lebar Kolom
    cols = [
        ("NO", 8), ("TANGGAL", 25), ("PROD NAME", 25), ("BATCH(KG)", 18), 
        ("RM WATER", 20), ("WATER (%)", 20), ("PH", 15), 
        ("DENSITY", 19), ("PARTICLE", 20), ("ACIDITY", 20)
    ]
    
    for col_name, width in cols:
        pdf.cell(width, 6, safe_text(col_name), 1, 0, 'C', fill=True)
    pdf.ln()
    
    # ISI TABEL DATA (ZEBRA STRIPING + KOMPRESI BARIS)
    pdf.set_font('Arial', '', 6.5)
    pdf.set_text_color(0, 0, 0)
    
    # Tinggi baris tabel dibuat 4.8 pixel agar menjamin 31 Hari MUAT di 1 Lembar (A4)
    row_height = 4.8 
    ...
    if df_pdf is not None and not df_pdf.empty:
        ...
        for i, row in df_pdf.reset_index(drop=True).iterrows():
            ...
            pdf.cell(cols[0][1], row_height, str(i+1), 1, 0, 'C', fill=True)
            pdf.cell(cols[1][1], row_height, safe_text(row['DATE']), 1, 0, 'C', fill=True)
            pdf.cell(cols[2][1], row_height, safe_text(row['PRODUCT']), 1, 0, 'C', fill=True)
            pdf.cell(cols[3][1], row_height, f"{row['BATCH']:,.0f}", 1, 0, 'C', fill=True)
            pdf.cell(cols[4][1], row_height, f"{row['MOIST_IN']:.2f}%", 1, 0, 'C', fill=True)
            pdf.cell(cols[5][1], row_height, f"{row['MOIST_FINISH_PRODUCT']:.2f}%", 1, 0, 'C', fill=True)
            pdf.cell(cols[6][1], row_height, f"{row['PH']:.2f}", 1, 0, 'C', fill=True)
            pdf.cell(cols[7][1], row_height, f"{row['DENSITY']:.4f}", 1, 0, 'C', fill=True)
            pdf.cell(cols[8][1], row_height, f"{row['PARTICLE_SIZE']:.2f}%", 1, 0, 'C', fill=True)
            pdf.cell(cols[9][1], row_height, f"{row['ACIDITY']:.3f}", 1, 0, 'C', fill=True)
            pdf.ln()
    ...
    # TANDA TANGAN (Lebih Naik ke Atas & Kanan)
    pdf.ln(4)
    pdf.set_font('Arial', '', 7.5)
    pdf.set_text_color(0, 0, 0)
    
    # X diset ke 130 agar posisi tanda tangan pas di kanan bawah
    pdf.set_x(130)
    pdf.cell(60, 4, safe_text(f'Dicetak pada: {datetime.datetime.now().strftime("%d-%m-%Y")}'), 0, 1, 'C')
    pdf.set_x(130)
    pdf.cell(60, 4, safe_text('Admin QC,'), 0, 1, 'C')
    
    # Jarak untuk Tanda Tangan disesuaikan agar tidak overpage
    pdf.ln(10) 
    ...
    if data_tersedia:
        ...
        with st.sidebar:
            ...
            if st.button("Generate PDF Report"):
                try:
                    pdf_bytes = create_pdf(sheet_name, df, total_prod_ton, achievement, forecast_msg)
                    ...
                    st.error(f"Gagal membuat PDF. Error Detail: {e}")
